...
#
# Files:
#   helpers/prepare-data.R  read + join the raw files; derive school-level
# variables
...
provider_choice_names <- lapply(names(PROVIDER_COLOURS), function(p) {
  tags$span(
    tags$span(
      class = "swatch",
      style = paste0("background:", PROVIDER_COLOURS[[p]])
    ),
...
cb_source <- function(file) {
  switch(file,
    eindscores = D$eind,
    referentieniveaus = D$ref,
    schooladviezen = D$adv,
    schoolweging = D$sw,
    derived = DERIVED_MID
  )
}
...
cb_header_stat <- function(row) {
  ...
  switch(row$kind,
    ...
    num = ,
    derived = paste0(
      fmt(100 * mean(!is.finite(as.numeric(x))), 1),
      "% missing"
    ),
...
sidebar_filters <- sidebar(
  ...
  radioButtons(
    ...
    choices = c(
      "the midpoint, 2.5" = "mid",
      "zero" = "zero",
      "missing: drop the school" = "drop"
    )
  ),
  ...
  radioButtons(
    "soort",
    "School type",
    c("All" = "All", "Regular (Bo)" = "Bo", "Special (Sbo)" = "Sbo"),
    inline = TRUE
  ),
  selectInput("province", "Province", c("All provinces" = "All", PROVINCES)),
  sliderInput(
    "min_n",
    "Minimum group-8 pupils tested",
    min = 0,
    max = 40,
    value = 0,
    step = 5
...
ui <- page_navbar(
  ...
  header = tags$head(tags$style(HTML("
    ...
    # f6f4ef;border-radius:6px;padding:8px 12px;margin:8px 0;font-size:.9rem}
    .gap-note{background:
    ...
    layout_columns(
      ...
      card(
        ...
        p(
          ...
          "Background and sources: data/doorstroomtoetsen-context.md and data/data-context.md."
        )
      ),
      card(
        ...
        gap_note(
          "Selection",
          "Schools chose their test. A raw provider comparison mixes a test effect with a school-population effect; the schoolweging tab tries to separate them."
        ),
        gap_note(
          "Consistency",
          "Raw scores are on six unrelated scales (IEP around 53-90, AMN around 316-456). Compare reference levels or advice instead."
        ),
        gap_note(
          "Derivation",
          "This is school-level, aggregated data. The Volkskrant probably had pupil-level data. A school mean is not a pupil."
        )
      )
    ),
    layout_columns(
      ...
      card(
        ...
        p(
          ...
          "Each extra requirement drops schools, and not at random. Grey bar = all schools in the file; dark bar = what is left."
        ),
      ...
      card(
        ...
        div(
          ...
          p(
            class = "small",
            "What is included or excluded, and does that match the reality I want to say something about? (Sbo schools have no schoolweging; exempted pupils aren't tested.)"
          )
        ),
        div(
          class = "principle",
          ...
          p(
            class = "small",
            "How do I handle it, and could it change the result? (Flip the “<5” setting in the sidebar and watch the numbers move.)"
          )
        ),
        div(
          class = "principle",
          ...
          p(
            class = "small",
            "Are the values reliable? (Counts are stored as text; provider scores live on different scales.)"
          )
        ),
        div(
          class = "principle",
          ...
          p(
            class = "small",
            "Do my computed variables capture what they claim? (“% HAVO+” is a choice of categories; a z-score within provider erases provider differences.)"
          )
        ),
        div(
          class = "principle",
          ...
          p(
            class = "small",
            "Did anything change across time, place or source? (Location codes differ between files; schoolweging exists per year and as a 3-year average.)"
          )
        )
      )
    )
  ...
  nav_panel(
    ...
    card(
      ...
      gap_note(
        "Derivation",
        "Each school is one observation, whether it tested 6 pupils or 130. Try the minimum-pupils slider: small schools produce the 0% and 100% spikes."
      )
    )
  ),
      ...
      card(
        ...
        layout_columns(
          ...
          selectInput(
            ...
            c(
              "All providers" = "All",
              setNames(PROVIDERS, paste("Only", PROVIDERS))
            )
          )
        ),
        layout_columns(
          ...
          checkboxInput(
            "s_trend",
            "Show binned means (10 equal-sized bins per colour)",
            TRUE
          )
        ),
        ...
        gap_note(
          "Selection",
          "Schools missing on either axis are silently dropped from a scatter plot. The count above tells you how many."
        )
      ),
      card(
        card_header("Selected school"),
        p(
          class = "small text-muted",
          "Click a point to see the raw counts behind it."
        ),
        uiOutput("school_detail"),
        plotOutput("school_bars", height = "300px")
      )
    )
  ),
  ...
  nav_panel(
    ...
    card(
      card_header(
        "The Volkskrant check: compare providers among similar schools"
      ),
      ...
      layout_columns(
        ...
        radioButtons(
          ...
          "Average",
          c(
            "Each school counts once" = "schools",
            "Weight by pupils" = "pupils"
          )
        ),
      ...
      gap_note(
        "Derivation",
        "Averaging within bands is a crude version of “controlling for” schoolweging. It only compares providers where both have enough schools, and it treats schoolweging as the only confounder. Region and denomination are also linked to provider choice (try the province filter)."
      ),
      gap_note(
        "Selection",
        "Sbo schools have no schoolweging and are always excluded here."
      )
    )
  ),
    ...
    card(
      ...
      layout_columns(
        ...
        radioButtons(
          ...
          c(
            "Provider" = "provider",
            "Province" = "PROVINCIE",
            "Denomination" = "DENOMINATIE_VESTIGING",
            "School type" = "SOORT_PO"
          ),
          inline = TRUE
        ),
      ...
      layout_columns(
        ...
        radioButtons(
          "g_axis",
          "Value axis",
          c("Include zero" = "zero", "Zoom to the data" = "fit"),
          inline = TRUE
        ),
        radioButtons(
          "g_sort",
          "Order",
          c("By value" = "value", "By number of schools" = "n", "A-Z" = "name"),
          inline = TRUE
        )
      ),
  ...
  nav_panel(
    ...
    layout_columns(
      ...
      card(
        ...
        gap_note(
          "Recording",
          "Schools are placed at the centre of their 4-digit postcode, so schools in one PC4 area sit on top of each other. Missing values are drawn as open grey circles instead of disappearing."
        )
      ),
      card(
        card_header("Selected school"),
        p(class = "small text-muted", "Click a school on the map."),
        uiOutput("school_detail_2"),
        plotOutput("school_bars_2", height = "300px")
      )
    )
  ),
  ...
  nav_panel(
    ...
    card(
      ...
      div(
        class = "cb-head cb-row",
        span("variable"),
        span("meaning"),
        span("file"),
        span("kind"),
        span("missing / suppressed")
      ),
      accordion(
        id = "codebook",
        open = FALSE,
  ...
  nav_spacer(),
  nav_item(
    tags$span(class = "navbar-text small", "DUO open data, school year 2024-25")
  )
)
...
server <- function(input, output, session) {
  ...
  click_scatter <- reactive(
    suppressWarnings(event_data("plotly_click", source = "scatter"))
  )
  click_map <- reactive(
    suppressWarnings(event_data("plotly_click", source = "map"))
  )
  ...
  dist_data <- reactive({
    ...
    b <- binned(
      ...
      grp,
      bins = input$d_bins,
      as_share = input$d_y == "share"
    )
    list(
      b = b |> group_by(group) |> filter(sum(n) > 0) |> ungroup(),
      d = d,
      grp = grp
    )
  })
  ...
  output$dist_plot <- renderPlotly({
    ...
    cols <- if (dd$grp == "provider") {
      PROVIDER_COLOURS
    } else if (dd$grp == "SOORT_PO") {
      TYPE_COLOURS
    } else {
      c(
        "All schools in view" = INK_2
      )
    }
    b <- dd$b |> mutate(
    ...
    p <- ggplot(b, aes(x = x, y = y, fill = group, text = tip)) +
      ...
      labs(
        x = var_label(input$d_var),
        y = if (input$d_y == "count") "schools" else "% of row"
      ) +
      theme_dash() +
  ...
  output$scatter <- renderPlotly({
    ...
    d <- d |>
      ...
      mutate(
        ...
        colour = factor(
          ifelse(focus, colour, "other"),
          levels = c(
            names(PROVIDER_COLOURS),
            names(TYPE_COLOURS),
            "all",
            "other"
          )
        ...
        tip = paste0(
          ...
          "</b> (",
          tools::toTitleCase(tolower(PLAATSNAAM)),
          ")<br>",
          provider,
          "<br>",
          var_label(x),
    ...
    p <- ggplot(d, aes(.data[[x]], .data[[y]])) +
      geom_point(
        aes(colour = colour, text = tip, customdata = id),
        alpha = 0.45,
        size = 1.4
      ) +
      scale_colour_manual(
        values = cols,
        name = NULL,
        breaks = setdiff(levels(droplevels(d$colour)), "other")
      ) +
  ...
  output$scatter_stat <- renderText({
    ...
    paste0(
      ...
      " schools  ·  ",
      format(sum(!ok), big.mark = ","),
      " schools in view dropped because x or y is missing"
    )
  })
  ...
  school_detail_ui <- function() {
    i <- selected()
    if (is.null(i)) {
      return(p(em("No school selected yet.")))
    }
    s <- derived()[i, ]
    used <- PROVIDERS[
      sapply(PROVIDERS, function(p) s[[paste0(p, "_AANTAL")]] != "0")
    ]
    tagList(
      ...
      p(
        class = "small text-muted",
        paste0(
          tools::toTitleCase(tolower(s$PLAATSNAAM)),
          " · ",
          s$PROVINCIE,
          " · ",
          s$SOORT_PO,
          " · ",
          s$DENOMINATIE_VESTIGING,
          " · ",
          s$INSTELLINGSCODE,
          "-",
          s$VESTIGINGSCODE
          ...
          lapply(
            ...
            function(p) {
              tags$tr(
                ...
                tags$td(paste0(s[[paste0(p, "_AANTAL")]], " pupils")),
                tags$td(
                  paste0("avg ", s[[paste0(p, "_GEM")]] |> parse_dec() |> fmt(1))
                )
              )
            }
          ),
          tags$tr(
            tags$td("Schoolweging (2024-25)"),
            tags$td(fmt(s$schoolweging, 2)),
            tags$td(as.character(s$sw_match))
          ),
          tags$tr(
            tags$td("Reading % 2F / maths % 1S"),
            tags$td(
              paste0(fmt(s$pct_lv_2F, 0), "% / ", fmt(s$pct_rek_1S, 0), "%")
            ),
            tags$td("")
          ),
          tags$tr(
            tags$td("Advice % HAVO+"),
            tags$td(paste0(fmt(s$pct_havo_plus, 0), "%")),
            tags$td("")
          )
        )
      )
  ...
  school_bars_plot <- function() {
    ...
    validate(
      need(nrow(bars) > 0, "No reference-level or advice rows for this school.")
    )
    pal <- c(
      low = "#e3a08a",
      mid = "#d6d2c8",
      target = "#86b6ef",
      setNames(colorRampPalette(c("#e8f0fb", "#86b6ef"))(10), short_adv)
    )
  ...
  vk_data <- reactive({
    ...
    cells <- d |>
      ...
      summarise(
        value = weighted.mean(.data[[out]], w),
        n = n(),
        .groups = "drop"
      ) |>
    ...
    list(
      ...
      band = band,
      xlim = if (nrow(cells)) {
        c(min(cells$band_lo), max(cells$band_lo) + band)
      } else {
        c(
          20,
          40
  ...
  output$vk_plot <- renderPlotly({
    ...
    p <- ggplot(cl, aes(band_mid, value, colour = provider)) +
      ...
      labs(
        x = "schoolweging band (higher = more disadvantaged pupil population)",
        y = var_label(input$v_out)
      ) +
      theme_dash()
    ggplotly(p, tooltip = "text") |>
  ...
  output$vk_comp <- renderPlot(res = 96, {
    ...
    comp <- d |>
      group_by(provider) |>
      summarise(
        p10 = quantile(schoolweging, .1),
        q1 = quantile(schoolweging, .25),
        med = median(schoolweging),
        q3 = quantile(schoolweging, .75),
        p90 = quantile(schoolweging, .9),
        n = n()
      ) |>
    ...
    ggplot(comp, aes(y = provider, colour = provider)) +
      ...
      geom_text(
        aes(x = p90, label = paste0("  n = ", n, ", median ", fmt(med, 1))),
        hjust = 0,
        colour = INK_2,
        size = 3.3
      ) +
  ...
  output$vk_gap <- renderUI({
    ...
    raw <- wm(a) - wm(b)
    both <- inner_join(
      filter(v$cells, provider == a),
      filter(v$cells, provider == b),
      by = "band_lo",
      suffix = c("_a", "_b")
    )
    ...
    layout_columns(
      col_widths = c(6, 6),
      div(
        div(class = "small text-muted", paste(a, "minus", b, ", all schools")),
        div(class = "big-num", sgn(raw))
      ),
      div(
        div(
          class = "small text-muted",
          paste0("... within equal-schoolweging bands (", nrow(both), " bands)")
        ),
        div(class = "big-num", sgn(within))
      ),
      p(
        class = "small text-muted",
        paste0(
          ...
          " ",
          fmt(sw(b), 1),
          ". If the two numbers above differ a lot, composition explains part of the raw gap."
        )
      )
    )
  })
  ...
  output$grp_plot <- renderPlot(res = 96, {
    ...
    st <- d |>
      ...
      summarise(
        mean = mean(.data[[v]]),
        q1 = quantile(.data[[v]], .25),
        med = median(.data[[v]]),
        q3 = quantile(.data[[v]], .75),
        n = n()
      )
    key <- switch(input$g_sort,
      value = if (input$g_mark == "bar") st$mean else st$med,
      n = st$n,
      name = -rank(st$group)
    )
    ...
    if (input$g_mark == "bar") {
      ...
      p <- p + geom_linerange(
        aes(xmin = q1, xmax = q3, colour = group),
        linewidth = 2.5,
        alpha = 0.5
      ) +
      ...
      p <- p + geom_jitter(
        data = d,
        aes(x = .data[[v]], y = group, colour = group),
        height = 0.25,
        width = 0,
        alpha = 0.25,
        size = 1
    ...
    p <- p + geom_text(
      aes(x = Inf, label = paste0("n = ", format(n, big.mark = ","))),
      hjust = 1.05,
      size = 3.3,
      colour = INK_2
    )
    lims <- if (input$g_axis == "zero") {
      c(min(0, rng[1]), rng[2])
    } else {
      rng + c(-0.15, 0.15) * diff(
        rng
      )
    }
    p + coord_cartesian(xlim = c(lims[1], lims[2] + 0.18 * diff(lims))) +
      labs(
        x = var_label(v),
        y = NULL,
        caption = if (input$g_mark == "strip") "One dot per school; vertical tick = median." else NULL
      ) +
      theme_dash(13) +
      theme(panel.grid.major.y = element_blank())
  })
  ...
  output$grp_warn <- renderUI({
    if (input$g_mark == "bar" && input$g_axis == "fit") {
      gap_note(
        "Visual encoding",
        "A bar encodes its value by its length. With the axis zoomed in, the bars' lengths no longer match the numbers, so small differences look huge. Dots and lines don't have this problem, because we read their position, not their length."
      )
    }
  })
  ...
  output$map <- renderPlotly({
    ...
    if (col == "provider") {
      p <- p + geom_point(
        aes(colour = provider, text = tip, customdata = id),
        size = 1.2,
        alpha = 0.8
      ) +
        scale_colour_manual(values = PROVIDER_COLOURS, name = NULL)
      ...
      p <- p + geom_point(
        ...
        geom_point(
          data = filter(d, is.finite(.data[[col]])),
          aes(colour = .data[[col]], text = tip, customdata = id),
          size = 1.2
        ) +
  ...
  lapply(seq_len(nrow(CODEBOOK)), function(i) {
    ...
    output[[paste0("cb_tab_", i)]] <- renderTable(
      {
        ...
        stat <- switch(row$kind,
          const = tibble(
            statistic = "value(s)",
            value = paste(unique(x), collapse = ", ")
          ),
          id = tibble(
            statistic = c("rows", "distinct values", "examples"),
            value = c(
              format(length(x), big.mark = ","),
              format(length(unique(x)), big.mark = ","),
              paste(head(unique(x), 3), collapse = " | ")
            )
          ),
          cat = {
            tb <- sort(table(x, useNA = "ifany"), decreasing = TRUE)
            tibble(statistic = names(tb), value = paste0(format(as.integer(tb), big.mark = ","), " (", fmt(100 * as.numeric(tb) / length(x), 1), "%)")) |> head(
              14
            )
          },
          count = {
            ...
            tibble(
              statistic = c(
                ...
                "zero",
                "“<5” (suppressed)",
                "reported numbers (0 or 5+)",
                "mean of reported numbers",
                "sd",
                "median",
                "IQR",
                "max"
              ...
              value = c(
                format(length(x), big.mark = ","),
                paste0(sum(x == "0"), " (", fmt(100 * mean(x == "0"), 1), "%)"),
                paste0(
                  sum(x == "<5"),
                  " (",
                  fmt(100 * mean(x == "<5"), 1),
                ...
                fmt(s$sd, 2),
                fmt(s$median, 1),
                paste(fmt(s$q1, 1), "to", fmt(s$q3, 1)),
                fmt(s$max, 0)
              )
            )
          },
          {
            ...
            tibble(
              statistic = c(
                "n (non-missing)",
                "missing",
                if (row$kind == "score") "0 (provider not used)",
                "mean",
                "sd",
                "min",
                "25th percentile",
              ...
              value = c(
                format(s$n, big.mark = ","),
                format(
                  s$missing - if (row$kind == "score") zero else 0,
                  big.mark = ","
                ),
                if (row$kind == "score") format(zero, big.mark = ","),
                fmt(s$mean, 2),
                fmt(s$sd, 2),
                fmt(s$min, 2),
                fmt(s$q1, 2),
    ...
    output[[paste0("cb_plot_", i)]] <- renderPlot(res = 96, {
      ...
      if (row$kind == "cat") {
        ...
        return(ggplot(tb, aes(n, value)) +
          geom_col(fill = INK_2, width = 0.7) +
          labs(x = "rows", y = NULL) +
          theme_dash())
      }
      if (row$kind == "count") {
        tb <- tibble(raw = x) |>
          count(raw) |>
          mutate(
            value = count_value(raw, "mid"),
            type = case_when(
              raw == "<5" ~ "“<5” (1-4, position unknown)",
              raw == "0" ~ "zero",
              TRUE ~ "reported count"
            )
          )
        return(ggplot(tb, aes(value, n, fill = type)) +
          ...
          scale_fill_manual(
            values = c(
              "reported count" = INK_2,
              "zero" = "#c4c1b8",
              "“<5” (1-4, position unknown)" = "#eb6834"
            ),
            name = NULL
          ) +
