...
4.  Read the background information and codebook for the data [here](../documents/data-resources.qmd). 
...
school_provider <- eindscores |>
  ...
  filter(!is.na(n_students), n_students > 0) |>           # drop unused/suppressed providers
  group_by(INSTELLINGSCODE, VESTIGINGSCODE) |>
  slice_max(n_students, n = 1, with_ties = FALSE) |>    # handles rare mixed-provider schools
  ungroup()
```
...
school_provider |>
  ...
  #filter(n_students > 50) |> # use this filter if plots get crowded with large municipalities 
  ggplot(aes(x = reorder(INSTELLINGSNAAM_VESTIGING, n_students), y = n_students, fill = provider)) +
  geom_col() +
  scale_fill_manual(values = provider_colors) + # the same named vector from above
