...
school_provider <- eindscores |>
  ...
  pivot_longer(cols = ends_with("_AANTAL"), names_to = "provider", values_to = "n_students") |>
  mutate(
    provider = sub("_AANTAL$", "", provider),                 # "IEP_AANTAL" -> "IEP"
    n_students = as.numeric(n_students),                          # DUO writes "<5" for small counts -> this becomes NA
    postcode = as.numeric(str_sub(POSTCODE_VESTIGING, 1, 4))  # keep only the numeric part: "2716PH" -> 2716
  ) |>
  filter(!is.na(n_students), n_students > 0) |>           # drop the providers a school didn't actually use
  group_by(INSTELLINGSCODE, VESTIGINGSCODE) |>        # group the rows back up by school...
  slice_max(n_students, n = 1, with_ties = FALSE) |>    # ...and keep only its single largest provider (most schools use just one anyway)
  ungroup()
