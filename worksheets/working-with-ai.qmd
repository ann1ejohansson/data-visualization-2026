...
!["Hi Chat! Can you generate an image that suits as an engaging catch, to put at the top of this worksheet?"](images/working-with-ai.png)
...
-   Check how robust the trend is by creating several visualizations of the same data. Also check its robustness across different data decisions (item 2).   
...
Decide your own verification criteria before you see the result, so that the AI's output doesn't set the standard it is judged by. For example:

-   Counts: how many rows (schools, pupils) went in, and how many ended up in the chart? You should be able to explain every difference.
-   Manual checks: reproduce one or two numbers from the chart (a count, a mean) manually. 
-   Plausibility: do the values fall within plausible ranges? Do they match what you know about the context?
-   Robustness: does the pattern survive a different chart type, a different summary statistic (median instead of mean), or a different filtering decision?
-   Independence: does a teammate, or a different model, reach the same result from the same question?
-   Understanding: can you explain the code?
...
8.  **AI is one tool among many.**  For example, AI can probably check whether your chosen color palette is color-blind friendly, but you are also [one click away](https://www.color-blindness.com/coblis-color-blindness-simulator/) from checking it yourself. 
...
-   [AI consumes resources](https://news.un.org/en/story/2026/06/1167658). How can you (now and in the future) approach this responsibly?  
